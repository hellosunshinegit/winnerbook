...
<html>
  ...
  <head>
	...
	<script type="text/javascript">
		...
		//添加
		function addFun(){
			window.location.href="${basePath }tagsController/addTags.html";
		}
  ...
  <body>
    <div class="page_title">
      ...
      <div class="page_main">
	    <form id="searchForm" name="searchForm" action="${basePath}tagsController/tagsList.html" method="post">
            ...
            <c:if test="${not empty pageDTO.data }">
				<c:forEach items="${pageDTO.data}" var="item" varStatus="status">
					<tr>
						...
						<td>${item.tagName}</td>
						<td><exp:code code="STATUS" value="${item.tagStatus }"></exp:code> </td>
						...
						<td>
							<c:if test="${sessionUser.isAdmin eq '1' or item.createUserId eq sessionUser.userId }">
								<a href="${basePath }tagsController/updateTags.html?tagId=${item.tagId}">修改</a>
							</c:if>
						</td>
					</tr>
				</c:forEach>
