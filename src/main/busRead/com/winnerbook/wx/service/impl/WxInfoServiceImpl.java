...
public class WxInfoServiceImpl extends BaseServiceImpl implements WxInfoService{
	...
	public String sendWbInfo(String idInfo,String accessToken) {
		...
		if("activity".equals(type)){//发送活动的数据
			...
			//发微博
			title = (StringUtils.isNotBlank(activity.getWbTitle())?activity.getWbTitle():activity.getTitle())+" "+ConstantUtils.H5_URL+"page/activity/activityDetail.html?busId="+user.getBelongBusUserId()+"&userId=&id="+activity.getId()+"&sourse=wb";
			...
			//发微博
			title = (StringUtils.isNotBlank(news.getWbTitle())?news.getWbTitle():news.getNewTitle())+" "+ConstantUtils.H5_URL+"page/detail/detail.html?busId="+user.getBelongBusUserId()+"&userId=&id="+news.getNewId()+"&list_type=2&sourse=wb";
			pic = news.getWbImg();
			...
			//发微博
			title = (StringUtils.isNotBlank(readBookClub.getWbTitle())?readBookClub.getWbTitle():readBookClub.getClubTitle())+" "+ConstantUtils.H5_URL+"page/detail/detail.html?busId="+user.getBelongBusUserId()+"&userId=&id="+readBookClub.getClubId()+"&list_type=3&sourse=wb";
			pic = readBookClub.getWbImg();
		}else if("course".equals(type)){
			Course course = courseService.findById(id);
			User user = userService.findUserById(course.getCreateUserId()+"");
			//发微博
			title = (StringUtils.isNotBlank(course.getWbTitle())?course.getWbTitle():course.getTitle())+" "+ConstantUtils.H5_URL+"page/detail/courseDetail.html?busId="+user.getBelongBusUserId()+"&userId=&courseId="+course.getCourseId()+"&type=1&sourse=wb";
			pic = course.getWbImg();
			...
			//发微博
			title = (StringUtils.isNotBlank(bookListType.getWbTitle())?bookListType.getWbTitle():bookListType.getTypeName())+" "+ConstantUtils.H5_URL+"page/list/bookList.html?busId="+user.getBelongBusUserId()+"&userId=&typeId="+bookListType.getId()+"&sourse=wb";
